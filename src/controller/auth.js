...
export const forgotPassword = async (req, res) => {
    try {
        ...
        let htmlRequest = await fs.readFileSync(`${__dirname}/../emailTemplates/forgotPassword.html`, 'utf8');
        let templates = Handlebars.compile(htmlRequest);
        const replacements = {
            token: `http://localhost:3000/auth/reset-password/${resetPasswordToken}`
        };
        const htmlToSend = templates(replacements);
        const options = {
